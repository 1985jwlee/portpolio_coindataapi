                ...
                while (_HttpListener.IsListening)
                {
                    ...
                    Task unawaited = Task.Run(async () =>
                    {
                        ...
                        catch (Exception eInner)
                        {
                            ...
                            var errorMsg = $"{ctx.Request.Url.Full} Exceptions {eInner.Message}/{eInner.StackTrace}";
                            Console.WriteLine(errorMsg);
                            if (ctx.Response.ChunkedTransfer)
                                await ctx.Response.SendChunk(Encoding.UTF8.GetBytes(errorMsg), true).ConfigureAwait(false);
                            else
                                await ctx.Response.Send(errorMsg).ConfigureAwait(false);
                        ...
                        finally
                        {
                            ...
                            if (ctx != null)
                            {
                                if (!ctx.Response.ResponseSent)
                                {
                                    ctx.Response.StatusCode = 500;
                                    ctx.Response.ContentType = "text/plain";
                                    var errorMsg = $"{ctx.Request.Url.Full} ctx is not null";
                                    if (ctx.Response.ChunkedTransfer)
                                        await ctx.Response.SendChunk(Encoding.UTF8.GetBytes(errorMsg), true).ConfigureAwait(false);
                                    else
